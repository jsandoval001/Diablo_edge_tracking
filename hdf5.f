...
!            write(*,*) ' Program: ', NX,(NY-1)*NPROCS+1,1
...
c$$$      write(230+RANK,'(1E)') TH(0:NX-1,0:NZ-1,1:NY,1)
      ...
      SUBROUTINE ReadHDF5_bisection( FNAME_TURB , FNAME_LAM , LAMBD )
      use hdf5
      ...
      CHARACTER*512 FNAME_TURB, FNAME_LAM
      LOGICAL FINAL
...
!     Create the file collectively
      call h5fopen_f(trim(FNAME_LAM), H5F_ACC_RDONLY_F,      
     &                 file_id, error, access_prp = plist_id_d)
      call h5pclose_f(plist_id_d, error)
...
!     Create the file collectively
      call h5fopen_f(trim(FNAME_TURB), H5F_ACC_RDONLY_F,      
     &                 file_id, error, access_prp = plist_id_d)
      call h5pclose_f(plist_id_d, error)
...
!     -----------------------------
      ...
      IF ( ABS( TIME_AUX - TIME ) > 1.0E-06 ) PRINT *, 'T0FILE ERROR!'
...
!     Close the properties for the reading
      ...
      end subroutine ReadHDF5_bisection
