...
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
     ...
     *       FFTW_BACKWARD, FFTW_MEASURE + FFTW_IN_PLACE )
      ...
      DO I=0,NKX
        DO K=0,NZ
          CZX_PLANE(K,I)=CMPLX(0.0d0,0.0d0)
        END DO
      END DO
      DO K=0,TNKZ
        DO J=0,NY
          CYZ_PLANE(J,K)=CMPLX(0.0d0,0.0d0)
        END DO
      END DO
...
C appropriately.
      ...
      DO J=JMIN,JMAX
        CALL FFT_X_TO_FOURIER(U,CU,J,J,0,NZM)
        IF (NZ.GT.1) THEN
        DO I=0,NKX
          DO K=0,NZM
...
C storage array and transform in the X direction using FFT_X_TO_PHYSICAL.
     ...
     *    CZX_PLANE(0,0), 1, NZ+1, CZX_PLANE(0,0), 1, NZ+1)
        ...
        END DO 
       END IF       
       CALL FFT_X_TO_PHYSICAL(CU,U,J,J,0,NZM)
      END DO
